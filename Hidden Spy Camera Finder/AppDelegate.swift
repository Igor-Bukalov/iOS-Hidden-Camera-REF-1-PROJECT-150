...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        HSCFIAPManager.shared.loadProductsFunc_HSCF()
        ThirdPartyServicesManager.shared.initializePushwooshHSCF(delegate: self) {}
        HSCFIAPManager.shared.validateSubscriptions_HSCF_S32HP(productIdentifiers: [Configurations_HSCF.mainSubscriptionID, Configurations_HSCF.unlockContentSubscriptionID, Configurations_HSCF.unlockFuncSubscriptionID]) { [weak self] result in
            if let unlockedContent = result[Configurations_HSCF.unlockFuncSubscriptionID], unlockedContent == true {
                HSCFIAPManager.shared.productBought.append(.unlockFuncProduct)
            }
            if let unlockedContent = result[Configurations_HSCF.unlockContentSubscriptionID], unlockedContent == true {
                HSCFIAPManager.shared.productBought.append(.unlockContentProduct)
            }
            DispatchQueue.main.async {
                if let userHaveSub = result[Configurations_HSCF.mainSubscriptionID] {
                    switch userHaveSub {
                    case true:
                    ...
                    case false:
                        DispatchQueue.main.asyncAfter(deadline: .now() + 2, execute: { [weak self] in
                            let unsubscribedVC = HSCFPremiumMainController()
                            unsubscribedVC.subscribed = {
    ...
    private func prepareAppearence() {
        ...
        navigationBarAppearance.titleTextAttributes = [.foregroundColor: UIColor.white, .font: UIFont.inter(.InterMedium, size: 20)]

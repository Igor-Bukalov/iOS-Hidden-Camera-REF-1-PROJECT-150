...
final class GSDA_ContainerForTabbarController_GSD: UIViewController {
    ...
    private func checkSubcription() {
        if !HSCFIAPManager.shared.productBought.contains(.unlockContentProduct) {
            antiSpy.tabImage.image = UIImage(systemName: "lock")
        } else {
            antiSpy.tabImage.image = TabController.antiSpy.icon
        }
        if !HSCFIAPManager.shared.productBought.contains(.unlockFuncProduct) {
            btRadar.tabImage.image = UIImage(systemName: "lock")
        } else {
            btRadar.tabImage.image = TabController.btRadar.icon
        }
    }
    ...
    private func actionTapped(controller: TabController) {
        ...
        case .antiSpy:
            if HSCFIAPManager.shared.productBought.contains(.unlockContentProduct) {
                ...
                unsubscribedVC.productBuy = .unlockContentProduct
                unsubscribedVC.subscribed = { [weak self] in
                    self?.actionTapped(controller: .antiSpy)
                }
                present(unsubscribedVC, animated: true)
                return
            }
        case .btRadar:
            if HSCFIAPManager.shared.productBought.contains(.unlockFuncProduct) {
                ...
                let unsubscribedVC = HSCFPremiumMainController()
                unsubscribedVC.modalPresentationStyle = .fullScreen
                unsubscribedVC.productBuy = .unlockFuncProduct
                unsubscribedVC.subscribed = { [weak self] in
